...
if len(means) == len(weights) == len(phi_nmr):
    ...
    for cutoff in [0.3, 50, 100]:
        means_filtered = means.where(means > np.log10(cutoff)).fillna(0)
        weights_filtered = weights.where(means > np.log10(cutoff))
        weights_filtered = (weights_filtered / weights_filtered.sum(axis=0)).fillna(0)

        perm_dict["Perm_GMM_"+str(cutoff)+"ms"] = (((phi_nmr*weights)**4) * ((10**means)**2)).sum(axis=1)
        # perm.rename("Perm_GMM", inplace=True)
        # perm.to_csv(os.path.join(output_folder, output_folder_name, "Permeability", well+"_NMRGMM.csv"), header=True)
