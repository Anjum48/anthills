...
class NMRGaussianProcess:
    def __init__(self, las_file, well, output_path, t2_min=0.3, t2_max=6000, plots=False, seed=None):
        ...
        if not os.path.exists(os.path.join(self.output_path, "NMR_GMM_Output")):
            os.makedirs(os.path.join(self.output_path, "NMR_GMM_Output"))
        if not os.path.exists(os.path.join(self.output_path, "NMR_GMM_Output", self.well)) and self.plots:
            os.makedirs(os.path.join(self.output_path, "NMR_GMM_Output", self.well))
        if not os.path.exists(os.path.join(self.output_path, "NMR_GMM_Output", "CSV")):
            os.makedirs(os.path.join(self.output_path, "NMR_GMM_Output", "CSV"))
        if not os.path.exists(os.path.join(output_folder, "NMR_GMM_Output", "Permeability")):
            os.makedirs(os.path.join(output_folder, "NMR_GMM_Output", "Permeability"))
    ...
    def make_plots(self, t2_dist, data, depth):
        ...
        fig.savefig(os.path.join(self.output_path, "NMR_GMM_Output", self.well, self.well + "_" + str(depth) + ".png"))
        # plt.show()
        plt.close()
    ...
    def run(self, start_depth=None, end_depth=None):
        ...
        # Save outputs
        means.to_csv(os.path.join(self.output_path, "NMR_GMM_Output", "CSV", self.well+"_means.csv"))
        variances.to_csv(os.path.join(self.output_path, "NMR_GMM_Output", "CSV", self.well+"_variances.csv"))
        weights.to_csv(os.path.join(self.output_path, "NMR_GMM_Output", "CSV", self.well+"_weights.csv"))
        perm.to_csv(os.path.join(output_folder, "NMR_GMM_Output", "Permeability", self.well+"_NMR_GMM.csv"), header=True)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(os.path.join(output_folder, "NMR_GMM_Output")):
        os.makedirs(os.path.join(output_folder, "NMR_GMM_Output"))
